...
class NoteGenerator:
    ...
    def _choose_best_frame(self, para: Paragraph, grid_path: Path) -> int:
        ...
        instruction = dedent(
            ...
            我将提供：
            1. 一段文字内容（表示视频或音频片段的文字摘要/解说）；  
            2. 一张九宫格图片（包含该时间范围内均匀截取的 9 帧截图，从左上到右下依次为第 1 至第 9 帧）。  
            
            请完成以下任务：
            - 分析九宫格中每个小图的内容、清晰度、构图与语义相关性；  
            - 过滤掉模糊、过渡、遮挡严重或字幕占比过高的帧；  
            - 在剩余帧中，选出最能代表段落语义和情绪的一张图片；  
            - **只返回这张小图对应的数字编号（1–9），不返回任何解释或其他文字。**
